...
function HtmlBuilder() {
    ...
    const TextAreaFunction = async (memberId, assessmentId, questionId) => {
        ...
        htmlValue = await sqlquery.GetCharacterAnswered(memberId, assessmentId, questionId);
        value = htmlValue.recordset.length > 0 ? htmlValue.recordset[0].AnswerText : '';
        return `<label for='${htmlValue.optionid}'></label> <textarea class='form-control' name='${questionId}' id='${htmlValue.ValueId}'>${value}</textarea>`;
    ...
    const RadioBoxFuction = async (memberId, assessmentId, questionId) => {
        ...
        if (optionid != 0) {
            ...
            if (data.recordset.length > 0) {
                ...
                for (const key in data.recordset) {
            ...
            const mydata = questionAnswers.recordset;
    ...
    this.VerifyOtp = async (memguid, OTP) => {

        let data = await sqlquery.OTPMemberVerification(memguid, OTP);
    ...
    this.transformData = async (data, memberId, assessmentId) => {
        try {
            ...
            for (const key in data) {
                ...
                const questionexists = _.findWhere(displaylist, { QuestionId: q.QuestionId })
